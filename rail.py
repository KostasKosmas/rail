...
def train_model(df, crypto_symbol):
    try:
        ...
        y = np.where(df["Close"].shift(-1) > df["Close"], 1, 0).ravel()  # Fix 1: Add .ravel()
...
def calculate_trade_levels(df, selector, model_rf, model_gb):
    try:
        ...
        high_low = df['High'].iloc[-14:] - df['Low'].iloc[-14:]  # Fix 2: Use .iloc
        atr = np.mean(high_low)
        
        # Current market state
        current_price = df['Close'].iat[-1]  # Fix 3: Use .iat for scalar value
        sma_trend = df['SMA_50'].iat[-1] > df['SMA_200'].iat[-1]  # Fix 4
